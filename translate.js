...
async function processPage(sourceUrl, fullDictionary, sortedKeys, imageReplacementMap, lastEditInfoState, forceTranslateList = []) {
    ...
    $contentContainer.find('a').each(function() {
        ...
        if (
            href && href.length > 1 && href.startsWith('/') && !href.startsWith('//') &&
            !href.includes(':') && !href.includes('#') &&
            !/\.(css|js|png|jpg|jpeg|gif|svg|ico)$/i.test(href)
        ) {
            const cleanPageName = href.substring(1);
            $(this).attr('href', `./${cleanPageName}.html`); 
        } 
        else if (href?.startsWith('/') && !href.startsWith('//')) {
            try { $(this).attr('href', new URL(href, BASE_URL).href); } catch(e) { console.warn(`[${pageName}] 无效的内部资源 href: ${href}`); }
        }
    });
